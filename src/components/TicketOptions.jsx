import { Issue } from "../pages/Issue";
import { useCategories } from "../hooks/useCategories";
import { Option } from "./Option";
import { Tag, Plus, MapPin } from "lucide-react";
import { useState, useEffect } from "react";
import { UseLocations } from "../hooks/useLocations";
export function TicketOptions() {
  const { loading: locationsLoading, locations } = UseLocations();
  const {
    categories,
    loading,
    updateCategory,
    createCategory,
    deleteCategory,
  } = useCategories();
  const [categoryName, setCategoryName] = useState("");
  const [isEditingCategory, setIsEditingCategory] = useState(false);
  const [editCategoryId, setEditCategoryId] = useState();
  return (
    <div className="bg-green-50">
      <div className="flex gap-8">
        <div className="w-full p-8 flex flex-col gap-8 bg-white m-8 rounded-xl border border-emerald-900">
          <div className="flex gap-4 items-center">
            <Tag className="h-12 w-auto bg-green-200  text-emerald-900 p-2 rounded-lg" />
            <div>
              <h1 className="font-semibold text-2xl">Ticket Categories</h1>
              <p className="font-light text-xl">
                Categories help route requestes to the right support area
              </p>
            </div>
          </div>
          <div className="border p-4  border-emerald-900 rounded-2xl flex flex-col gap-4">
            <div className="w-full">
              <h1 className="font-semibold text-lg">Category Name</h1>
              <input
                className="border-emerald-900 border h-10 focus:outline-none p-1 text-lg rounded-lg w-full"
                placeholder="e.g. Printer Issue"
                value={categoryName}
                onChange={(e) => setCategoryName(e.target.value)}
              ></input>
            </div>
            <div className="flex gap-4">
              <button className="flex cursor-pointer gap-2 h-10 p-1  pl-4 pr-4 justify-center bg-sky-600 rounded-lg text-white text-lg items-center">
                <Plus />
                <p
                  className="text-center"
                  onClick={() => {
                    if (isEditingCategory) {
                      updateCategory(editCategoryId, categoryName);
                    } else {
                      createCategory(categoryName);
                    }
                    setIsEditingCategory(false);
                    setCategoryName("");
                  }}
                >
                  {isEditingCategory ? "Save Changes" : "Add Category"}
                </p>
              </button>
              {isEditingCategory && (
                <button className="h-10 p-1 pl-4 pr-4 cursor-pointer font-semibold justify-center bg-green-200  rounded-lg  text-lg items-center">
                  Cancel
                </button>
              )}
            </div>
          </div>
          <div className="bg-white  rounded-xl flex flex-col border border-emerald-900">
            {loading ? (
              <h1>loading</h1>
            ) : (
              categories.map((category, index) => {
                return (
                  <Option
                    key={category.id}
                    isFirst={index === 0}
                    optionTitle={category.category}
                    optionId={category.id}
                    setIsEditing={setIsEditingCategory}
                    setEditId={setEditCategoryId}
                    setEditName={setCategoryName}
                    deleteOption={deleteCategory}
                  />
                );
              })
            )}
          </div>
        </div>
        <div className="bg-white p-4 m-4 w-full">
          {loading ? (
            <h1>loading</h1>
          ) : (
            categories.map((category, index) => {
              return <h1 key={category.id}>{category.category}</h1>;
            })
          )}
        </div>
      </div>
      <div>
        <h1 className="text-center text-4xl font-bold border-b border-emerald-900 p-4 m-8">
          Preview
        </h1>
        <Issue />
      </div>
    </div>
  );
}
